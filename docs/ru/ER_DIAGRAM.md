**Язык / Language / Тіл:** [Русский](ER_DIAGRAM.md) · [English](../en/ER_DIAGRAM.md) · [Қазақша](../kk/ER_DIAGRAM.md)

# ER-диаграмма и модель данных

## Принципы моделирования

1. **Source of truth:** нормализованные доменные таблицы в PostgreSQL.
2. **Raw immutable:** `raw_documents` — append-only (доказательная база для аудита).
3. **Spatial first:** геометрии только через PostGIS (`geometry` / `geography`).
4. **ML reproducibility:** каждый `risk_assessment` хранит `model_version` + `feature_vector` (JSONB).
5. **Multi-country:** `country_code` на всех сущностях закупок.
6. **Soft delete** только для пользовательских сущностей; закупочные данные — versioned upsert.
...
Пороги — в конфиге `risk_engine`, не хардкод в UI.

---

## Миграции

- Один Alembic на сервис **или** shared migration runner с schema-per-service:
  - MVP: **одна БД, схемы:** `tender`, `market`, `geo`, `risk`, `ingest`, `iam`
  - Scale: физическое разделение БД при сохранении логических схем
...
Это позволяет позже вынести схемы в отдельные инстансы без переписывания домена.
