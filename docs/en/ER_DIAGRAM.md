**Language:** [Русский](../ru/ER_DIAGRAM.md) · [English](ER_DIAGRAM.md) · [Қазақша](../kk/ER_DIAGRAM.md)

# ER Diagram and Data Model

## Modeling Principles

1. **Source of truth:** normalized domain tables in PostgreSQL.
2. **Raw immutable:** `raw_documents` — append-only (evidentiary base for audit).
3. **Spatial first:** geometries only via PostGIS (`geometry` / `geography`).
4. **ML reproducibility:** each `risk_assessment` stores `model_version` + `feature_vector` (JSONB).
5. **Multi-country:** `country_code` on all procurement entities.
6. **Soft delete** only for user entities; procurement data — versioned upsert.
...
Thresholds — in `risk_engine` config, not hardcoded in the UI.

---

## Migrations

- One Alembic per service **or** a shared migration runner with schema-per-service:
  - MVP: **one DB, schemas:** `tender`, `market`, `geo`, `risk`, `ingest`, `iam`
  - Scale: physical DB split while keeping logical schemas
...
This allows splitting schemas into separate instances later without rewriting the domain.
